...
public sealed class EnemyHealth2D : MonoBehaviour,IDamageable2D
{
    ...
    [Header("EXP 풀(필수)")]
    [Tooltip("ExpOrb 풀. 비워두면 런타임에 자동 생성(프로토타입용)")]
    ...
    private void Awake()
    {
        ...
        if (expOrbPool == null)
        {
            var go = new GameObject("ExpOrbPool");
            expOrbPool = go.AddComponent<ExpOrbPool>();
        }

        // 미리 좀 만들어두면 드랍 순간 GC/스파이크 줄어듦
